...
export default {
  async fetch(req, env) {
    ...
    if (action === 'submitMessage') {
      ...
      if (rec.role === 'judge') {
        const jlist = (snap.settings && snap.settings.judges) || [];
        const mm = jlist.find(j => j.email && rec.username && j.email.toLowerCase() === rec.username.toLowerCase());
        if (mm) myName = mm.name;
        const onBench = (c.bench || []).some(b => b && b.name === myName);
