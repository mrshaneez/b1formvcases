...
function chatReadChannelsForRole(role) {
  ...
  if (role === 'judge') return ['bench', 'registry'];
  ...
  async fetch(req, env) {
    ...
    if (action === 'pullData') {
      ...
      if (rec.role === 'judge') {
        ...
        return json({ ok: true, data: { cases: myCases.map(c => caseForRole(c, 'judge')), contacts: myContacts, partyRequests: myReqs, targets: myTargets, deletedTargetIds: snap.deletedTargetIds || [], deletedCaseIds: snap.deletedCaseIds || [], settings: safeSettings, roster, judge: true, updatedAt: snap.updatedAt } });
      }
