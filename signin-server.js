...
export default {
  async fetch(req, env) {
    ...
    if (action === 'pushCases') {
      ...
      // Merge hearing/deadline-related fields from the judge's copy into the authoritative case.
      incoming.forEach(ic => {
        if (!ic || !ic.id) return;
        const sc = byId[ic.id];
        if (!sc) return;                       // never create/replace whole cases here
        sc.nextHearing = ic.nextHearing || '';
        sc.hearingTime = ic.hearingTime || '';
        if (Array.isArray(ic.timeline)) sc.timeline = ic.timeline;
        if (Array.isArray(ic.log)) sc.log = ic.log;
        if (Array.isArray(ic.applications)) sc.applications = ic.applications;
      });
      snap.cases = server;
      snap.updatedAt = new Date().toISOString();
      await K.put('data:registry', JSON.stringify(snap));
