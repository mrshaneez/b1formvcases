  ...
  async fetch(req, env) {
    ...
    if (action === 'pullData') {
      ...
      if (rec.role === 'judge') {
        ...
        return json({ ok: true, data: { cases: myCases, contacts: myContacts, partyRequests: myReqs, targets: myTargets, deletedTargetIds: snap.deletedTargetIds || [], settings: safeSettings, judge: true, updatedAt: snap.updatedAt } });
      }
    ...
    if (action === 'pushCases') {
      ...
      incoming.forEach(ic => {
        if (!ic || !ic.id) return;
        const sc = byId[ic.id];
        if (!sc) return;                       // never create/replace whole cases here
        // Staff/admin may update any case; a judge only cases they sit on.
        const allowed = (rec.role === 'staff' || rec.role === 'admin') || onBench(sc);
        if (!allowed) return;
        JUDGE_FIELDS.forEach(f => { if (Object.prototype.hasOwnProperty.call(ic, f)) sc[f] = ic[f]; });
      });
      snap.cases = server;
      snap.updatedAt = new Date().toISOString();
      await K.put('data:registry', JSON.stringify(snap));
